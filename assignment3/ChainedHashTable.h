...
void ChainedHashTable<T>::resize() {
	d = 1;
	while (1<<d <= n / lf) d++; // d is based on size plus loadfactor
    n = 0;
...
bool ChainedHashTable<T>::add(T x) {
	...
	if (n+1 > (t.length * lf)) resize(); // resize if more values than loadfactor * t.length
	t[hash(x)].add(x);
	n++;
	return true;
}
...
template<class T>
void ChainedHashTable<T>::SetLoadFactor(float f) { // change loadfactor
  lf = f;
  resize(); // resize based on new loadfactor
}

template<class T>
DLList<T> ChainedHashTable<T>::GetLongestList() {
  DLList<T> list_to_return;

  for (int i = 0; i < t.length; i++) { // iterate though DLLists
    if (t[i].size() > list_to_return.size()) { // check which one is bigger
      list_to_return = t[i];
    }
  }

  return list_to_return; // return first instance of biggest list
}
